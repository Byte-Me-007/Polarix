"""
Sequence Preparation & Normalization Pipeline for Polarix Maitri LSTM Autoencoder.

Key features:
- Chronological per-sensor 70/15/15 split.
- Strict leakage prevention: Mean and Std computed exclusively from NORMAL training records.
- Sliding window sequence creation (seq_len=30).
- Explicit missing / dropout handling: NaN sequences are never trained.
- Metadata preservation (station_id, sensor_id, timestamp, is_anomaly, anomaly_type).
"""
...
class SensorScaler:
    ...
    @classmethod
    def from_dict(cls, data: Dict[str, Any]) -> "SensorScaler":
        return cls(
...
def fit_sensor_scalers(
    train_df: pd.DataFrame, min_std: float = 1e-6
) -> Dict[str, SensorScaler]:
    """
    Compute normalization parameters (mean, std) ONLY from NORMAL training records.

    Parameters:
    -----------
    train_df : pd.DataFrame
        Training partition of telemetry data.
    min_std : float
        Numerical epsilon to avoid divide-by-zero on low variance sensors.

    Returns:
    --------
    Dict[str, SensorScaler]: Scaler mapping per sensor_id.
    """
    scalers = {}
    for sensor_id, group in train_df.groupby("sensor_id"):
        # Strictly select non-null NORMAL records only
        ...
        scalers[sensor_id] = SensorScaler(
            sensor_id=sensor_id,
            mean=mean_val,
            std=std_val,
            unit=unit,
...
def save_scalers(scalers: Dict[str, SensorScaler], output_path: Path) -> None:
...
def load_scalers(input_path: Path) -> Dict[str, SensorScaler]:
    """Load scalers from a JSON file."""
    with open(input_path, "r", encoding="utf-8") as f:
        data = json.load(f)
    return {k: SensorScaler.from_dict(v) for k, v in data.items()}
...
def extract_sliding_windows(
    ...
    """
    Generate sliding windows for a single sensor.
    ...
    for i in range(n - seq_len + 1):
        ...
        if np.isnan(window_vals).any():
            # Skip sequence if it contains dropout
            continue

        if normal_only:
            # All points in the window must be normal
            if np.any(window_anom != 0):
                continue

        # Normalize sequence using scaler fitted on train normal data
        norm_window = scaler.transform(window_vals)

        # Target metadata maps to the LAST timestamp in the sliding window
...
def prepare_maitri_datasets(
    ...
    - Splits per sensor chronologically: 70% train, 15% val, 15% test.
    - Fits scalers strictly on NORMAL training records.
    - Extracts training sequences (normal only).
    - Extracts validation & test sequences (all valid windows for anomaly evaluation).

    Returns:
    --------
    Dict containing:
        - 'train_sequences': np.ndarray (N_train, seq_len, 1)
        - 'val_sequences': np.ndarray (N_val, seq_len, 1)
        - 'test_sequences': np.ndarray (N_test, seq_len, 1)
        - 'train_metadata': List[Dict]
        - 'val_metadata': List[Dict]
        - 'test_metadata': List[Dict]
        - 'scalers': Dict[str, SensorScaler]
    """
    df = pd.read_csv(csv_path)

    # Sort per sensor chronologically
    try:
    ...
    # Chronological partition per sensor
    ...
    # Fit scalers on NORMAL training partition ONLY
    scalers = fit_sensor_scalers(train_df)

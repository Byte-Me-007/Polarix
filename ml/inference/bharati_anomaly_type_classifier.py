"""
Bharati Telemetry Anomaly Type Classifier (Polarix SIH26060 - Person C).

A transparent, deterministic, feature-based rule engine that categorizes
detected Bharati telemetry anomaly windows into explainable physical classes:
...
- Calibrated and validated specifically for the 5 Bharati sensor channels.
"""
...
# Default sensor-specific noise std for Bharati sensors ('BRT')
DEFAULT_BHARATI_SENSOR_NOISE_STD: Dict[str, float] = {
    "BRT_TEMP_001": 0.30,
    "BRT_PRESS_001": 0.20,
    "BRT_HUM_001": 1.00,
    "BRT_VIB_001": 0.03,
...
class BharatiWindowFeatures:
    """Extracted statistical features for explainable Bharati anomaly classification."""
...
class BharatiAnomalyTypeClassifier:
    """
    Explainable signal classifier for 30-step Bharati telemetry windows.
    """
    ...
    def __init__(
        ...
        drift_trend_ratio_threshold: float = 0.40,
        spike_jump_ratio_threshold: float = 3.0,
    ) -> None:
    ...
    def extract_features(self, window: Union[List[float], np.ndarray]) -> BharatiWindowFeatures:
        ...
        if not np.all(np.isfinite(arr)):
            raise ValueError("Window contains non-finite (NaN or Inf) values.")
        ...
        if n >= 3:
            ...
            if t_var > 0 and std_val > 0:
                ...
                denom = np.sqrt(t_var) * np.sqrt(np.sum(arr_centered**2))
                r_val = float(np.sum(t_centered * arr_centered) / denom) if denom > 0 else 0.0
            else:
                slope = 0.0
                r_val = 0.0
        else:
        ...
        # Count consecutive identical or near-stuck values across entire window
        max_stuck_run = 0
        current_run = 1
        for i in range(1, n):
    ...
    def classify(
        ...
        is_sudden_jump = (
            ...
            and feats.max_step_jump >= 2.0 * noise_std
        )
        has_isolated_pulse = (
            feats.max_step_jump >= 0.35 * feats.total_variation
            and feats.trend_ratio < 0.85
            and feats.max_step_jump >= 2.5 * noise_std
        )

        if is_sudden_jump or has_isolated_pulse:

...
export const TwinLegend = ({ sensors = [], isHeatmapActive = false }) => {
  ...
  const legendItems = isHeatmapActive
    ? [
        { label: 'NORMAL', color: '#3f6e4a', count: normal, desc: 'Nominal' },
        { label: 'WARNING', color: '#b26814', count: warning, desc: 'Caution' },
        { label: 'CRITICAL', color: '#b5382b', count: critical, desc: 'Active Alert' },
        { label: 'OFFLINE', color: '#5d6672', count: offline, desc: 'No Data' }
      ]
    : [
  ...
  return (
    <div className="twin-legend-panel">
      <span className="twin-legend-title">
        {isHeatmapActive ? 'SPATIAL CONDITION' : 'SENSOR TELEMETRY STATUS'}
      </span>
